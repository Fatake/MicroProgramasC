...
MatrixMultiply(int n, float *a, float *b, float *c)
{
	int i, j, k;
	for (i=0; i<n; i++)
		for (j=0; j<n; j++)
...
int main(int argc, char *argv[]){
    ...
    float *c = create_array_as_matrix(N, N);
	...
	float bloque_a[nro_procesos][n_chica*n_chica];
	float bloque_b[nro_procesos][n_chica*n_chica];
	float bloque_c[nro_procesos][n_chica*n_chica];
